...
def create_dataset(rawpath, modelpath, calibpath):
    ...
    # Model 
    Model_Molecular_Backscatter_532 = xr.open_dataset(modelpath)['beta532'].values #pd.read_pickle(modelpath)['beta532mol'].unstack(level=1).values
    Model_Molecular_Backscatter_355 = xr.open_dataset(modelpath)['beta355'].values#pd.read_pickle(modelpath)['beta355mol'].unstack(level=1).values
    ...
    attrs_calibration_heigth = xr.open_dataset(calibpath).attrs['calibration_height']
...
CALIB_DIR = Path('/homedata/nmpnguyen/IPRAL/NETCDF/v_simple/', str(year))

if __name__== 'main' :
    parser = optparse.OptionParser()
    parser.add_option('-file', help='path of ipral raw file to input', dest='inputfile', action='store')
    (opts, args) = parser.parse_args()
    rawpath = Path(opts['inputfile'])
    print(rawpath)

    calibpath = sorted(CALIB_DIR.glob(f'{rawpath.name}'))[0]
    modelpath = Path('/homedata/nmpnguyen/database_lidars/tmp_simul.nc')
    ...
    for rawpath in sorted(RAW_DIR.glob('**/**/ipral_1a_Lz1R15mF30sPbck_v01_202001*_000000_1440.nc')): 
