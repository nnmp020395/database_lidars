...
def simulate_Rayleight_coef(output_era, wavelengths):
    """
    Input is the output dataframe of variables_from_era function 
    wavelengths = list of wavelength
    """
    
    print('-----SIMULATE EXTINCTION & BACKSCATTER COEFF. FROM ERA5-----')
    print('List of wavelength', wavelengths)
    # set constants
    #----------------------------------------------------------------------------------
    k = 1.38e-23
    ...
    for wavelength in wavelengths:
        ...
        beta_name = 'beta'+str(wavelength)
        alpha_name = 'alpha'+str(wavelength)
        const = (5.45e-32/1.38e-23)*((wavelength*1e-3/0.55)**(-4.09))
        output_era[beta_name] = const*output_era['pression'].div(output_era['ta'])
        # compute extinction coef
        #----------------------------------------------------------------------------------
        output_era[alpha_name] = output_era[beta_name]*ratio
...
def interpolate_atb_mol(file, era, wavelengths):     
    """
    ...
    #----------------------------------------------------------------------------------
    # new_df = xr.Dataset(
    #         coords = dict(time = ('time', timeData), range = ('range', r)),)
    # print('-----INTERPOLATE ATTENUATED BACKSCATTERING FROM ERA5-----')
    # for wavelength in wavelengths:
    #     beta_name = 'beta'+str(wavelength)
    #     alpha_name = 'alpha'+str(wavelength)
    #     columns_names = ['altitude', 'pression', 'ta', beta_name, alpha_name]
    #     pression_interp, ta_interp, beta_interp, alpha_interp = [[] for _ in range(len(columns_names)-1)]
    ...
    #         f_alpha = spi.interp1d(a['altitude'], a[alpha_name], kind = 'linear', bounds_error=False, fill_value="extrapolate")
    #         f_beta = spi.interp1d(a['altitude'], a[beta_name], kind = 'linear', bounds_error=False, fill_value="extrapolate")
    #         f_pression = spi.interp1d(a['altitude'], a['pression'], kind = 'linear', bounds_error=False, fill_value="extrapolate")
    #         f_ta = spi.interp1d(a['altitude'], a['ta'], kind = 'linear', bounds_error=False, fill_value="extrapolate")
    #         alpha_interp = np.append(alpha_interp, np.array(f_alpha(r)))
    #         beta_interp = np.append(beta_interp, np.array(f_beta(r)))
    #         pression_interp = np.append(pression_interp, np.array(f_pression(alt))), 
    #         ta_interp = np.append(ta_interp, np.array(f_ta(alt)))

    #     new_df = new_df.assign(beta_name = )          
        


    columns_names = ['altitude', 'pression', 'ta', 'beta355', 'beta532', 'alpha355', 'alpha532', 'beta387', 'alpha387']#, 'beta355mol', 'beta532mol', 'tau355', 'tau532', 'beta355mol', 'beta532mol'
    pression_interp, ta_interp, beta355_interp ,beta532_interp ,alpha355_interp ,alpha532_interp, beta387_interp, alpha387_interp = [[] for _ in range(len(columns_names)-1)] #, beta355mol_interp ,beta532mol_interp ,tau355_interp ,tau532_interp , beta355mol_interp ,beta532mol_interp, 
    new_index = pd.MultiIndex.from_product([timeData, r], names = ['time', 'range'])
    ...
    for t1 in tqdm(time_tmp):
        ...
        f3 = spi.interp1d(a['altitude'], a['beta387'], kind = 'linear', bounds_error=False, fill_value="extrapolate")
        f4 = spi.interp1d(a['altitude'], a['alpha387'], kind = 'linear', bounds_error=False, fill_value="extrapolate")
        ...
        beta387_interp, alpha387_interp = np.append(beta387_interp, np.array(f3(r))), np.append(alpha387_interp, np.array(f4(r)))
        pression_interp, ta_interp = np.append(pression_interp, np.array(f9(alt))), np.append(ta_interp, np.array(f10(alt)))
    print(pression_interp)
    #------
    new_df = pd.DataFrame(index = new_index, 
        data = np.array([pression_interp, ta_interp, alpha355_interp, alpha532_interp, beta355_interp, beta532_interp, beta387_interp, alpha387_interp]).T, 
...
def main_simulate(file):
    ...
    variables_Rayleight_computed = simulate_Rayleight_coef(variables_from_era_data, [355, 532, 387])
    new_netcdf = interpolate_atb_mol(file, variables_Rayleight_computed, [355, 532, 387])
    if os.path.exists('/homedata/nmpnguyen/database_lidars/tmp_simul.nc'):
        os.remove('/homedata/nmpnguyen/database_lidars/tmp_simul.nc')
    new_netcdf.to_netcdf('/homedata/nmpnguyen/database_lidars/tmp_simul.nc', 'w')
    return 0
